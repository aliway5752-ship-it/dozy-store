    ...
    try {
      ...
      try {
        ...
        const orderData = {
          ...
          landmark: formData.landmark,
          notes: formData.notes,
          totalAmount: cart.items.reduce((sum, item) => sum + (Number(item.price) * item.quantity), 0) + shippingPrice,
...
📍 Building/Floor/Apartment: ${formData.addressDetails}
...
*[Total Amount]*
💰 Total: ${cart.items.reduce((sum, item) => sum + (Number(item.price) * item.quantity), 0) + shippingPrice} EGP
          `.trim()
        };
        ...
        const botResponse = await fetch(targetEndpoint, {
          method: 'POST',
          mode: 'cors',
          headers: {
